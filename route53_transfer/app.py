...
def get_zone(con, zone_name, is_private):

    res = con.get_all_hosted_zones()
    zones = res['ListHostedZonesResponse']['HostedZones']
    zone_list = [z for z in zones
                    if z['Config']['PrivateZone'] == (u'true' if is_private else u'false')
                        and z['Name'] == zone_name + '.']

    if len(zone_list) > 0:
        zone_response = zone_list[0]
        data = {}
        data['id'] = zone_response['Id'].replace('/hostedzone/', '')
        data['name'] = zone_response['Name']
        return data
    else:
        return None


def create_zone(con, zone_name, is_private, vpc_region, vpc_id):
    vpc={}
    if is_private:
        vpc['region'] = vpc_region
        vpc['id'] = vpc_id
        if not vpc['region'] or not vpc['id']:
            exit_with_error("ERROR: Private zone {} does not exist -- creating a new private zone "
                            "requires a VPC Region and ID (--vpc-region, --vpc-id)".format(zone_name))
    ...
    return get_zone(con, zone_name, is_private)
...
def load(con, zone_name, file_in, is_private, vpc_region, vpc_id):
    ''' Send DNS records from input file to Route 53.
    
        Arguments are Route53 connection, zone name, file open for reading, and if zone is private (public is default).
    '''
    zone = get_zone(con, zone_name, is_private)
    if not zone:
        zone = create_zone(con, zone_name, is_private, vpc_region, vpc_id)
...
def dump(con, zone_name, fout, is_private):
    ...
    zone = get_zone(con, zone_name, is_private)
    if not zone:
        exit_with_error("ERROR: {} zone {} not found!".format('Private' if is_private else 'Public',
                                                              'zone_name'))
...
def run(params):
    ...
    is_private = params.get('--private')
    vpc_region = params.get('--vpc-region')
    vpc_id = params.get('--vpc-id')

    if params['dump']:
        dump(con, zone_name, get_file(filename, 'w'), is_private)
    elif params['load']:
        load(con, zone_name, get_file(filename, 'r'), is_private, vpc_region, vpc_id)
    else:
        return 1
