...
class ComparableSameNameRecord():
    def __init__(self, obj):
        for k, v in obj.__dict__.items():
            self.__dict__[k] = v
...
def load(con, zone_name, file_in, **kwargs):
    ...
    to_upsert = desired_records.intersection(existing_records)
    to_add = desired_records.difference(to_upsert)
    ...
    if to_add or to_delete or to_upsert:

        precommit = ResourceRecordSets(con, zone['id'])
        changes = ResourceRecordSets(con, zone['id'])

        aliases = set()

        for record in to_add:
            is_alias = record.alias_dns_name is not None
            if is_alias:
                aliases.add((record.alias_hosted_zone_id, record.alias_dns_name))
        ...
        for record in to_upsert:
            is_alias = record.alias_dns_name is not None
            if is_alias:
                print ('UPSERT', record.name, record.type, record.alias_dns_name)
            else:
                print ('UPSERT', record.name, record.type, record.resource_records)

            current_batch = precommit if (zone['id'], record.name) in aliases \
                else changes

            change = current_batch.add_change('UPSERT', **record.to_change_dict())
        ...
        for record in to_add:
            is_alias = record.alias_dns_name is not None
            if is_alias:
                print ('CREATE', record.name, record.type, record.alias_dns_name)
            else:
            ...
            change = current_batch.add_change('CREATE', **record.to_change_dict())
            for value in record.resource_records:
                change.add_value(value)

        current_batch = precommit if precommit.changes else changes

        for record in to_delete:
            change = current_batch.add_change('DELETE', **record.to_change_dict())
        ...
        if dry_run:
            ...
            if precommit.changes:
                print('There are changes that need to be committed before the main batch')
                for c in precommit.changes:
                    print('  - ', *c)
            if changes.changes:
                print('There are other rr changes:')
                for c in changes.changes:
                    print('  - ', *c)
        else:
            if precommit.changes:
                print('Applying pre-commit changes...')
                precommit.commit()
            print('Applying main changes...')
            changes.commit()
        print ("Done.")
    else:
        print ("No changes.")
